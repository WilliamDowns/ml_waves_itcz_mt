'''
Get all Atlantic tropical waves from Tropical Weather Discussion files
'''
...
# set year bounds
#years = [str(year) for year in range(2004,2023)]
#years = [str(2004)]
#years = [str(2023)] #GO THROUGH CODE AND REMOVE 2023 BITS IF RERUNNING FULL YEAR SET
years = [str(2014)] #GO THROUGH CODE AND REMOVE 2014 BITS IF RERUNNING FULL YEAR SET
# set month bounds, recognizing that files often come out before midnight
min_month = '06'
max_month = '11'
...
#def assign_filename_dates(year):
for year in years:
    ...
    path = os.path.join(topdir,'TWDs/archive/text/TWDAT/', year)    
    yearly_filenames = os.listdir(path)

    # 1. Remove TWDs that aren't needed and have bad timesteps (manually checked based on following 2 conditions)    
    for f in global_skip_TWD_files:
        if f[6:10] == year:
            try:
    ...
    for f in yearly_filenames:
        ...
        if not found_corresponding_time:
            ...
            if not ((closest_time == '00' and ((part_of_day == 'PM' and timestamp_hour_str in ('06', '07', '08'))\
                                               or (part_of_day == 'UTC' and timestamp_hour_str in ('23', '00'))))\
                    or (closest_time == '06' and ((part_of_day == 'AM' and timestamp_hour_str in ('12', '01', '02'))\
                                                  or (part_of_day == 'UTC' and timestamp_hour_str in ('05', '06'))))\
                    or (closest_time == '12' and ((part_of_day == 'AM' and timestamp_hour_str in ('06', '07', '08'))\
                                                  or (part_of_day == 'UTC' and timestamp_hour_str in ('11', '12'))))\
                    or (closest_time == '18' and ((part_of_day == 'PM' and timestamp_hour_str in ('12', '01', '02'))\
                                                  or (part_of_day == 'UTC' and timestamp_hour_str in ('17', '18'))))):
                ...
                try:
                    timestamp_UTC_match = correct_timestamps_dict[(part_of_day, timestamp_hour_str)]
                except:
                    # catch any files with ambiguous timestamps that haven't manually been weeded out
                    print('ISSUE FILE REMAINS:', f)
                    continue
                ...
                #print('Reassigning file ' + f + ' with timestamp ' + str((part_of_day, timestamp_hour_str)) + \
                #      ' to ' + yearly_dates_by_filename[f])

                #print('first read time: ' + first_time, ', second read time: ' + second_time,
                #      ', publication hour: ' + timestamp_hour_str,', part of day: ' + part_of_day,
                #      ', expected analysis time: ' + check_str, ', expected TWD time: ' + closest_time)
                #
                #print('Did not find corresponding time in ' + f + ' with matched time ' + \
                #      yearly_dates_by_filename[f] + '. The surface analysis line states: ' + time_line)
        
    filenames.extend(yearly_filenames)
    dates_by_filename.update(yearly_dates_by_filename)
    #return yearly_filenames, yearly_dates_by_filename


'''
yearly_vals = Parallel(n_jobs=-1)(delayed(assign_filename_dates)(year)
                                  for year in years)

yearly_filename_lists, yearly_date_filename_dicts = [], []

for pair in yearly_vals:
    yearly_filename_lists.append(pair[0])
    yearly_date_filename_dicts.append(pair[1])

for l in yearly_filename_lists:
    filenames.extend(l)

for d in yearly_date_filename_dicts:
    dates_by_filename.update(d)
'''

# add back in our manually assigned times    
dates_by_filename.update({k: v for k,v in global_manual_assignments.items() if 'AT' in k})
filenames.extend([k for k in global_manual_assignments.keys() if 'AT' in k])
filenames.sort()
...
for date in filenames_by_date.keys():
    #if int(date[4:6]) < 6 or int(date[4:6]) > 11:
    #    continue
    filenames_by_date[date] = filenames_by_date[date][-1]
    # debug odd surface analysis timesteps
...
for time in filenames_by_date.keys():
    ...
    path = os.path.join(topdir,'TWDs/archive/text/TWDAT/', f[6:10])
    ...
    #for line in no_waves_block:
    #    if 'wave' in line.lower():
    #        print('Found misplaced tropical waves in ', f)
    #        break
...
#wave_counts_by_year = {year : 0 for year in years}
#word_counts={}
...
location_re = '(' + '|'.join([s.lower().replace(' ', '[ ]*') for s in global_loc_keyword_map.keys()-['Mexico', 'gulf of mexico', 'mexico']]) + ')'
#location_re = '([/-]?' + '|'.join([s.lower().replace(' ', '[ ]*') for s in global_loc_keyword_map.keys()-['Mexico', 'gulf of mexico']]) + ')'
...
# want "s of", "south of", " southward"
lat_vector_re = '(?: s|south|southward|equatorward)[ ]*(?:of|from)[ ]*{}(?:n|s)|[, ]+{}(?:n|s)[ ]*(?:southward|south|equatorward)'.format(c_re, c_re)
# don't want "s of XXw"
lat_vector_typo_re = '(?: s|south)[ ]*(?:of|from)[ ]*{}w'.format(c_re)
...
#def get_waves_by_time(time):
for time in filenames_by_date.keys():    
    blocks = wave_blocks_by_time[time]
    #if time == '201910260000':
    #    print('201910260000', blocks)
    ...
    for p in paragraphs:
        if 'waves' in p and not exclude_by_string(p, 'wave', exclusion_pairs):
            #print(time, p)
            True
    paragraphs = [p for p in paragraphs if exclude_by_string(p, 'wave', exclusion_pairs)]
    
    #if len(paragraphs) > 0:
    #    wave_counts_by_year[time[:4]] = wave_counts_by_year[time[:4]] + len(paragraphs)
    ...
    for p in paragraphs:
            ...
            for loc, lat in zip(raw_lat_locs, raw_lats):
                ...
                if not repeat:
                    ...
                    if 'southward' in sentence and len(lat_coords) == 0:
                        ...
                        #if 'south' in sentence or 'from ' + lat in sentence:
                        #if len(re.findall('from[ ]*{}'.format(lat), sentence)) > 0:
                        #    print(time, lat_coords, pre_wave_sentence, wave_sentence, post_wave_sentence)
        ...
        if len(verbal_locations) + len(coord_pairs) + len(lat_coords) + len(lon_coords) > 2 and\
           ...
           not all([len(re.findall('of|from|south', lat)) > 0 for lat in lat_coords]):
            ...
            waves.append(TwdWave(time, coord_pairs, lat_coords, lon_coords, verbal_locations, 2, p))
            continue
            #print(time, coord_pairs, lat_coords, lon_coords, verbal_locations,
            #      'pre:', pre_wave_sentence, 'wave:', wave_sentence, 'post:', post_wave_sentence)
    ...
    #return time, waves, time_metrics
    #if len(waves) == 0 and int(time[4:6]) in (6,7,8,9,10):# and int(time[:4] == '2010'):
        #print(time)#, blocks)
    #    True
    ...
    for wave in waves:
        ...
        for wave2 in waves:
            ...
            if np.abs(wave_lon-wave2_lon) < 1 and wave2.trust > wave.trust:
                waves.remove(wave2)
        #if (abs(wave.lon_range[0] - wave.lon_range[1]) > 9 or min(wave.extent[0][0], wave.extent[1][0]) < -5\
        #    or max(wave.extent[0][0], wave.extent[1][0]) > 30) and wave.trust not in (1,6,7):
        #    #print(wave, pre_wave_sentence, wave_sentence, post_wave_sentence)
        #    #wave.full_print()
        #    True
        #if (wave.extent[0][0] + wave.extent[1][0]) / 2 > 20:
        #    wave.full_print()
    waves_by_time[time] = waves + waves_by_time[time]
    #waves_by_time[time].extend
    for key in metrics:
...
# save merge waves
with open(os.path.join(topdir, 'merge_waves.csv'+'_2014'), 'w') as f:
    write = csv.writer(f)
    write.writerow(merge_fields)
    write.writerows(merge_rows)
        

#waves_and_times = Parallel(n_jobs=-1)(delayed(get_waves_by_time)(time)
#                                      for time in filenames_by_date.keys())

#for wave_set in waves_and_times:
#    waves_by_time[wave_set[0]] = wave_set[1]
#    for key in metrics:
#        metrics[key] = metrics[key] + time_metrics[key]


#print('total waves:', sum(wave_counts_by_year.values()))
print(metrics)

#exit()

# Save raw waves (no lat assignments done yet for non trust==1 waves)
with open(os.path.join(topdir, 'raw_waves'+'_2014'), 'wb') as f:
    pickle.dump(waves_by_time, f, protocol=pickle.HIGHEST_PROTOCOL)
#END COMMENT BLOCK FOR RAW WAVE CALCULATIONS


with open(os.path.join(topdir, 'raw_waves'+'_2014'), 'rb') as f:
...
for time in waves_by_time.keys():
    if int(time[4:6]) in (5,6,7,8,9,10,11):
        ...
        #calendar_days[time[4:8]] = calendar_days[time[4:8]] + [wave in waves_by_time[time] if wave.trust == 1]
...
# interpolate existing data to all dates / lons
print(np.nanmean(climo_lats))
print(climo_lat_sums)
print(climo_lats)
#print(climo_lats)
with open(os.path.join(topdir, 'wave_lat_climo'), 'wb') as f:
    pickle.dump(climo_lats, f, protocol=pickle.HIGHEST_PROTOCOL)
#END COMMENT BLOCK FOR CLIMO LAT CALCULATIONS

'''
with open(os.path.join(topdir, 'wave_lat_climo'), 'rb') as f:
    climo_lats = pickle.load(f)

    
# assign lat values to waves that need it
# comment block below for latitude reassignment and 
...
for time in waves_by_time.keys():
    ...
    for wave in waves_by_time[time]:
        if int(time[4:6]) in (5,6,7,8,9,10,11):
            #init_lats = (np.round(wave.extent[0][0], 2), np.round(wave.extent[1][0], 2))
            wave.assign_lat(climo_lats)
            #new_lats = (np.round(wave.extent[0][0], 2), np.round(wave.extent[1][0], 2))
            #if init_lats != new_lats:# and len(wave.verbal_locations) + len(wave.coord_pairs) + len(wave.lat_coords) == 1:
            #    print(wave, init_lats, new_lats)
        new_waves.append(wave)
    #print([str(wave) for wave in waves_by_time[time]])
...
with open(os.path.join(topdir, 'final_waves'+'_2014'), 'wb') as f:
    pickle.dump(waves_by_time, f, protocol=pickle.HIGHEST_PROTOCOL)
#END COMMENT BLOCK FOR LAT

with open(os.path.join(topdir, 'final_waves'+'_2014'), 'rb') as f:
    waves_by_time = pickle.load(f)
...
# NOTE FOR WAVE ID CODE: 201907031030.txt has the following sentence:
# "The previously analyzed western Caribbean tropical wave with axis along 80W is presently
# not identifiable. as it has lost its signature"
# NOTE: 201409291800 lists coordinate location in second sentence
# NOTE: 201206301800 lists coordinate location in second sentence
# NOTE: 200908040600 declares a portion of a wave to now be a trough
# NOTE: 200808100000 moves a wave location from coordinate pair (first) to word location
# NOTE: 200709050000 mentions low spawned form a wave
# NOTE: 200706130600 mentions itcz precipitation associated with a wave
# be sure not to double count waves
# should "open wave"s be counted?

# NOTE: WAVE LOCATIONS ARE ALWAYS 6 HOURS OLD
'''
envisioned flow of the wave location identifier:
1. Pick location based on where it is in the paragraph
   a. Is there an identifier in the same sentence as the word wave? Prioritize this.
      Should be after the word wave but before the next sentence.
   b. Is there an identifier in the sentence after the word wave? Check this next.
   Rules for locations not to include:
       a. Precip, convection
       b. low pressure along the wave (This can be used as a second-order location selector)
2. Prioritize location data based on type as follows:
   1. Sets of coordinate pairs (18n58w, [20n65w 14n64w])
   2. Singular coordinates (57w s of 20n, along 58w south of 25n)
   3. Land location orientation (south of Puerto Rico, between Venezuela and Cuba)
      (May want to blacklist these descriptors and only use days without them, or
       only use waves with real descriptors from those days and don't train off these geographical
       descriptor wave locations when picking negative space areas to train off of)


Quality control: Check if a wave has a length listed of less than a certain distance, or with a
west-east angle / spread that is unusually high
ALSO, PRIORITIZE DEFINED LONGITUDES?
'''
...
manual check list
-------------------------
201507011200, 2, [[], ['15n'], ['32W/33W'],  'southward']
201809290600
'''
